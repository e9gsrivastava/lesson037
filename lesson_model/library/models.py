...
class Book(models.Model):
    ...
    genre=models.CharField(max_length=30, choices=[('Horror','horror'), ('Self Help','self help'), ('Adventure','adventure'), ('Others','others')],default='Others')
    ...
    def delete_book(cls, book_id):
        book = cls.objects.filter(pk=book_id).first()
        if book:
            book.delete()
    ...
    def soft_delete_book(cls, book_id):
        book = cls.objects.get(pk=book_id).first()
        if book:
